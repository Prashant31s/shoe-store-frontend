...
const ProductDetails  = ({product ,products}) => {
  ...
  const notify =() => {
    toast.success('Great! Product added to cart', {
         ...
         <Wrapper>
            <div className="flex flex-col lg:flex-row md:px-10 gap-[50px] lg:gap-[100px]">
                ...
                <div className="flex-[1] py-3">
                        ...
                        <div id= "sizesGrid" className="grid grid-cols-3 gap-2">
                            {p.size.data.map((item,i)=>(
                              <div key ={i} className={`border rounded-md text-center py-3 
                                  font-medium ${
                                    item.enabled ? "hover:border-black cursor-pointer" : "cursor-not-allowed bg-black/[0.15] opacity-50"} ${selectedSize===item.size ? "border-black ":""}`}
                    ...
                    active:scale-95 mb-3 hover:opacity-75 "  onClick={()=>{
                          if(!selectedSize){
                            setshowError(true)
